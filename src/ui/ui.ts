...
export class UI {
  ...
  constructor(private action: (id: string) => void) {
    ...
    this.app.innerHTML = [
      ...
      '<section id="map-panel" class="map-panel" hidden aria-labelledby="map-title"><div class="eyebrow">THE CITY AT A GLANCE</div><h2 id="map-title">Find your wonder.</h2><button id="start-exploring" class="primary full map-start" data-action="walk" aria-label="Start exploring">' + icon('walk') + '<span><strong>Start exploring</strong><small>Walk in first person</small></span>' + icon('arrow') + '</button><div class="landmark-list">' + LANDMARKS.map((l, i) => '<button class="landmark-item" data-action="landmark:' + l.id + '" aria-label="Go to ' + l.name + '"><span class="landmark-number">0' + (i + 1) + '</span><span><strong>' + l.name + '</strong><small>' + l.artifact + '</small></span>' + icon('arrow') + '</button>').join('') + '</div><div id="map-description" class="map-description" role="status">Choose a place to arrive at its entrance.</div></section>',
