...
export class PlanarExhibition {
  ...
  constructor(readonly id: string, parent: THREE.Group, x: number, z: number, colliders: ColliderSpec[], interactives: Interactive[]) {
    ...
    this.pieces.forEach((piece, i) => {
      ...
      colliders.push({ type: 'box', position: [x + site.x, floor + .23 + (site.y === undefined && id !== 'station' ? .16 : 0), z + site.z], size: [width * .35, .08, .24], yaw: site.yaw });
      ...
      tasks.push(photograph(piece.photos[0].thumb ?? piece.photos[0].file).then((map) => {
        ...
        picture.geometry.dispose(); picture.geometry = new THREE.PlaneGeometry(size.width, size.height); const material = picture.material as THREE.MeshBasicMaterial; material.color.set('#ffffff'); material.map = map; material.needsUpdate = true; if (backPicture) backPicture.geometry = picture.geometry;
