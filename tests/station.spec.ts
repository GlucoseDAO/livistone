...
test('enter the Embryo ring, explore the station, discover its story, and resume from its map card', async ({ page }) => {
  ...
  expect(after.mode).toBe('walking'); expect(after.position.x).toBeCloseTo(-16, 2); expect(after.position.z).toBeCloseTo(-56, 2); expect(after.yaw).toBe(0);
...
test('station map and discovery are usable on a touch viewport', async ({ browser }) => {
  ...
  try {
    ...
    expect((await snapshot(page)).position.x).toBeCloseTo(-16, 2); expect((await snapshot(page)).position.z).toBeCloseTo(-56, 2); expect(errors).toEqual([]);
