...
for (const mobile of [false, true]) test(`integrated garden stories, walking and map preservation (${mobile ? 'touch' : 'desktop'})`, async ({ browser }) => {
  ...
  try {
    ...
    await page.reload(); await expect(page.getByRole('button', { name: 'Map', exact: true })).toBeEnabled({ timeout: 60000 }); await page.getByRole('button', { name: 'Map', exact: true }).click(); expect((await snapshot(page)).zone).toBe('town'); expect((await snapshot(page)).progress.discovered).toContain('living-dewdrop'); expect(errors).toEqual([]);
