...
for (const mobile of [false, true]) test(`textured railway and mountain passage render and remain traversable on ${mobile ? 'touch' : 'desktop'}`, async ({ browser }) => {
  ...
  try {
    ...
    for (const name of ['ballast', 'sleeper', 'rail']) for (const suffix of mobile ? ['color', 'roughness'] : ['color', 'normal', 'roughness']) expect(textures.has(`${name}-${suffix}.jpg`)).toBe(true);
    ...
    await page.getByRole('button', { name: 'City map' }).click(); await page.getByRole('button', { name: 'Return to walking' }).click();
