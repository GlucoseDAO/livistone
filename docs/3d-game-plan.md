...
No `.blend`, `.glb`, `.gltf`, `.obj`, `.fbx`, `.stl`, `.3dm`, `.step`, `.stp`, or `.ply` files were found in the two repositories during the initial inventory. The user will provide STL files and, if needed, original Grasshopper definitions later. Start the blockout from the photographs and concept; original-file delivery does not block movement, map, layout, or asset-pipeline development.
...
Desktop controls use mouse look, WASD and arrow-key movement, an interaction key, and Escape to release the pointer and open the menu. Capture the pointer only after a player action. Offer drag-to-look if pointer capture is declined, adjustable sensitivity, and a reset-position action. Disable head bob and motion blur by default. [Three.js pointer-lock controls](https://threejs.org/docs/pages/PointerLockControls.html)
