...
The Mycelium grove follows the ring’s two catalogue photographs: tall branching silver stems support curled, pierced mushroom crowns around opalescent centers. Rain drips through selected folds toward the visible rill and basin; the full crowns clear the dry loop. Mobile reduces the number of mushrooms and the detail of their folds. Three source-linked stories remain available. Map, journal and menu preserve walking position and facing; reload returns to the southern station exit and retains read stories. See [the revised implementation record](concepts/13-integrated-town/notes.md).
